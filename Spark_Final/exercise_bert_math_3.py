...
- Dynamic masking (finds '=' and masks the rest)
...
# --------------------------------------------------------------------
# Checkpoint & Overfitting Manager (Same as before)
# --------------------------------------------------------------------
class CheckpointManager:
    ...
    def save(self, params, opt_state, step, metrics, is_best=False):
        ...
        if is_best:
            ...
            print(f"  [ckpt] New best (val_acc={self.best_metric:.1f}%)")
...
def create_masked_batch(equations, rng_key, mask_prob=0.8):
    ...
    for i, eq in enumerate(equations):
        ...
        # Find result positions dynamically
...
# --------------------------------------------------------------------
# Probe: Full result accuracy
# --------------------------------------------------------------------
def probe_accuracy(params, val_eqs, num_samples=300):
    fully_correct = 0
    total = 0
    ...
    for _ in range(num_samples):
        ...
        is_correct = True
        for pos in res_pos:
            if int(jnp.argmax(logits[0, pos])) != ids[pos]:
                is_correct = False
                break
        total += 1
        if is_correct: fully_correct += 1

    return {"full_result_acc": 100.0 * fully_correct / total if total else 0}
...
def train_bert(train_eqs, val_eqs, steps=5000, resume_from=None, save_dir="checkpoints/bert_math"):
    ...
    default_history = {'train_loss': [], 'val_loss': [], 'val_acc': [], 'steps': []}
    ...
    print(f"\n{'='*65}\nTraining: steps {start_step}->{steps} | Task: +, - & Negatives\n{'='*65}\n")
    ...
    for chunk in range(start_step // chunk_size, total_chunks):
        ...
        if (chunk + 1) % 5 == 0:
            val_results = probe_accuracy(params, val_eqs, 300)
            val_loss = compute_val_loss(params, val_eqs, 20)
            auditor.update_probe_scores(val_results)

            history['train_loss'].append(train_loss)
            history['val_loss'].append(val_loss)
            history['val_acc'].append(val_results['full_result_acc'])
            history['steps'].append(global_step)

            should_stop, status = overfit_detector(train_loss, val_results['full_result_acc'])
            print(f"Chunk {chunk+1:3d} (step {global_step:5d}): train={train_loss:.4f} val={val_loss:.4f} | Val_Acc={val_results['full_result_acc']:5.1f}% | {status}")
    ...
    ckpt_mgr.save(params, opt_state, global_step, {'val_loss': val_loss, **val_results}, is_best)
    ckpt_mgr.save_history(history)
    print(f"\nDone. Best val accuracy: {ckpt_mgr.best_metric:.1f}%")
...
if __name__ == "__main__":
    ...
    if not args.skip_train:
        ...
        train_eqs = generate_equations((1, 50), 50000, ops=['+', '-'], allow_negatives=True)
        val_eqs   = generate_equations((1, 50), 5000, ops=['+', '-'], allow_negatives=True)
